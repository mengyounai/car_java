...
function initModel(a, c) {
    ...
    try {
        ...
        }) : "unlimited" == live2d_settings.waifuDraggable ? $(".waifu").draggable({revert: live2d_settings.waifuDraggableRevert}) : $(".waifu").css("transition", "all .3s ease-in-out")
    } catch (e) {
...
function loadTipsMessage(a) {
    function c() {
        var a = g("modelId"), b = live2d_settings.modelRandMode;
        $.ajax({
            cache: "switch" == b,
    ...
    function e() {
        var a = g("modelId"), b = g("modelTexturesId"), d = live2d_settings.modelTexturesRandMode;
        $.ajax({
            cache: "switch" == d,
    ...
    $.each(a.seasons, function (a, b) {
        a = new Date;
        var d = b.date.split("-")[0], c = b.date.split("-")[1] || d;
        d.split("/")[0] <= a.getMonth() + 1 && a.getMonth() + 1 <= c.split("/")[0] && d.split("/")[1] <= a.getDate() && a.getDate() <= c.split("/")[1] && (b = getRandText(b.text), b = b.render({year: a.getFullYear()}), showMessage(b, 6E3, !0))
    });
...
live2d_settings.waifuEdgeSide = "right:150";
